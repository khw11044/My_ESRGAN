...
def train_fn(
    ...
    for idx, (low_res, high_res) in enumerate(loop):
        ...
        if idx % 100 == 0 and idx>0:
            print('test')
            plot_examples("test_images/", gen) # idx=idx
...
def main():
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        if config.COLAB and epoch%5==0:
            target_folder = '/content/my_ESRGAN/checkpoint/'
            gen_file = sorted(glob.glob(target_folder + '/*.pth'))[-2]
            opt_file = sorted(glob.glob(target_folder + '/*.pth'))[-1]
